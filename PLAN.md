...
- The tracked Git checkout is being prepared for the `1.2.3` WordPress 7.0 compatibility release.
...
- OpenAI model filtering now explicitly allows GPT-5.5-compatible model IDs when they are returned by the user's OpenAI API key.
- WordPress.org readme tags are limited to the five supported tags: `images`, `seo`, `alt-text`, `openai`, and `gemini`.
- WordPress.org readme metadata now declares `Tested up to: 7.0`.
- The release package is generated by `scripts/build-dist.sh` as `oneclickcontent-images.zip` with a top-level `occidg/` folder.
- Validation in the current release pass: `php -l`, `npm run fix`, `npm run check`, `npm run test:local`, and `npm run dist` all passed.
- `npm run plugin-check` is still blocked locally because WP-CLI does not have the Plugin Check command installed.
- The remaining source-control step is to commit, push, create the GitHub release, and monitor the WordPress.org deployment workflow.

## Next recommended steps
1. Commit and push the `1.2.3` compatibility release from `/home/jameswilson/.openclaw/workspace/projects/oneclickcontent/repos/oneclickcontent-images`.
2. Publish the `1.2.3` GitHub release so the WordPress.org deployment workflow runs.
3. Monitor the GitHub Actions deploy job and verify the WordPress.org release after it completes.
4. Browser-smoke-test the Settings, Bulk Edit, and attachment-edit generation flows on a working WordPress site.
...
- `1.2.1` adds explicit forward-compatible OpenAI model filtering for GPT-5.5 API model IDs while keeping the dropdown sourced from the user's actual OpenAI API model list.
- The GitHub Actions deploy job now extracts `oneclickcontent-images.zip` into a clean `dist/` directory before handing it to `10up/action-wordpress-plugin-deploy`.
